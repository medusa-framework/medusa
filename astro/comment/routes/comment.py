...
@login_required
def create():
    return Utils().to_json(BaseController(Comment()).create())


@comment.route("/factory", methods=["POST"])
@login_required
def factory():
    return Utils().to_json(BaseController(Comment()).factory())


@comment.route("/get/all", methods=["GET"])
def get_all():
    return Utils().to_json(BaseController(Comment()).get_all())


@comment.route("/get", methods=["GET"])
def get():
    return Utils().to_json(BaseController(Comment()).get())


@comment.route("/delete/all", methods=["DELETE"])
@login_required
def delete_all():
    return Utils().to_json(BaseController(Comment()).delete_all())


@comment.route("/delete", methods=["DELETE"])
@login_required
def delete():
    return Utils().to_json(BaseController(Comment()).delete())


@comment.route("/update/all", methods=["PATCH"])
@login_required
def update():
    return Utils().to_json(BaseController(Comment()).update_all())


@comment.route("/update", methods=["PATCH"])
@login_required
def update_all():
    return Utils().to_json(BaseController(Comment()).update())
