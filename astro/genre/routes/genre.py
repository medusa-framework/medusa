...
@login_required
def create():
    return Utils().to_json(BaseController(Genre()).create())


@genre.route("/seed", methods=["POST"])
@login_required
def seed():
    return Utils().to_json(GenreController().seed())


@genre.route("/get/all", methods=["GET"])
def get_all():
    return Utils().to_json(BaseController(Genre()).get_all())


@genre.route("/get", methods=["GET"])
def get():
    return Utils().to_json(BaseController(Genre()).get())


@genre.route("/delete/all", methods=["DELETE"])
@login_required
def delete_all():
    return Utils().to_json(BaseController(Genre()).delete_all())


@genre.route("/delete", methods=["DELETE"])
@login_required
def delete():
    return Utils().to_json(BaseController(Genre()).delete())


@genre.route("/update/all", methods=["PATCH"])
@login_required
def update():
    return Utils().to_json(BaseController(Genre()).update_all())


@genre.route("/update", methods=["PATCH"])
@login_required
def update_all():
    return Utils().to_json(BaseController(Genre()).update())
