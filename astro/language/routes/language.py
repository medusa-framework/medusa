...
@login_required
def create():
    return Utils().to_json(BaseController(Language()).create())


@language.route("/seed", methods=["POST"])
@login_required
def seed():
    return Utils().to_json(LanguageController().seed())


@language.route("/get/all", methods=["GET"])
def get_all():
    return Utils().to_json(BaseController(Language()).get_all())


@language.route("/get", methods=["GET"])
def get():
    return Utils().to_json(BaseController(Language()).get())


@language.route("/delete/all", methods=["DELETE"])
@login_required
def delete_all():
    return Utils().to_json(BaseController(Language()).delete_all())


@language.route("/delete", methods=["DELETE"])
@login_required
def delete():
    return Utils().to_json(BaseController(Language()).delete())


@language.route("/update/all", methods=["PATCH"])
@login_required
def update():
    return Utils().to_json(BaseController(Language()).update_all())


@language.route("/update", methods=["PATCH"])
@login_required
def update_all():
    return Utils().to_json(BaseController(Language()).update())
